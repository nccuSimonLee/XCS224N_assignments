...
def naive_softmax_loss_and_gradient(
    ...
    ### YOUR CODE HERE
    y_hat = softmax(np.matmul(outside_vectors, center_word_vec))
    loss = -np.log(y_hat[outside_word_idx])
    y_hat[outside_word_idx] -= 1
    grad_center_vec = np.matmul(y_hat, outside_vectors)
...
def skipgram(current_center_word, window_size, outside_words, word2ind,
    ...
    for outside_word in outside_words:
        cur_loss, cur_dv, cur_do = word2vec_loss_and_gradient(center_word_vec,
                                                              word2ind[outside_word],
                                                              outside_vectors,
                                                              dataset)
        loss += cur_loss
        grad_center_vecs[word2ind[current_center_word]] += cur_dv
        grad_outside_vectors += cur_do
    ### END YOUR CODE
